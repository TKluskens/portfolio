...
function PrivateBadge({ client }: { client: string }) {
  return (
    <span style={{
      ...
      border: "1px solid oklch(30% 0.01 240)",
...
function LibraryDetail({ project, onClose }: { project: AnyProject; onClose: () => void }) {
  const accent = "oklch(48% 0.13 5)";
  const bg     = "oklch(12% 0.01 5)";
  const surface= "oklch(16% 0.012 5)";
  const border = "oklch(24% 0.012 5)";
  const muted  = "oklch(62% 0.01 5)";
  const p = project as PublicProject;

  return (
    <div style={{ minHeight: "100vh", background: bg, color: "oklch(90% 0.006 5)", fontFamily: "var(--ff-body)", overflowY: "auto" }}>
      <div style={{ borderBottom: `1px solid ${border}`, padding: "1.25rem 2.5rem", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: bg, zIndex: 10 }}>
        <BackButton onClose={onClose} color={accent} />
        <span style={{ fontFamily: "var(--ff-mono)", fontSize: "0.65rem", color: accent, letterSpacing: "0.1em" }}>SCHOOL LIBRARY SYSTEM · 2023</span>
        <GitHubLink url={p.sourceLink} accent={accent} />
      </div>
...
function TurtleDetail({ project, onClose }: { project: AnyProject; onClose: () => void }) {
  const accent = "oklch(62% 0.16 250)";
  const bg     = "oklch(10% 0.01 240)";
  const surface= "oklch(14% 0.012 240)";
  const border = "oklch(22% 0.01 240)";
  const muted  = "oklch(60% 0.01 240)";
  const p = project as PublicProject;

  return (
    <div style={{ minHeight: "100vh", background: bg, color: "oklch(88% 0.008 240)", fontFamily: "var(--ff-body)", overflowY: "auto" }}>
      <div style={{ borderBottom: `1px solid ${border}`, padding: "1.25rem 2.5rem", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: bg, zIndex: 10 }}>
        <BackButton onClose={onClose} color={accent} />
        <span style={{ fontFamily: "var(--ff-mono)", fontSize: "0.65rem", color: accent, letterSpacing: "0.1em" }}>TURTLE SRL DASHBOARD · 2023</span>
        <GitHubLink url={p.sourceLink} accent={accent} />
      </div>
...
function RecipeDetail({ project, onClose }: { project: AnyProject; onClose: () => void }) {
  const accent = "oklch(68% 0.18 45)";
  const bg     = "oklch(12% 0.01 40)";
  const surface= "oklch(16% 0.012 40)";
  const border = "oklch(24% 0.012 40)";
  const muted  = "oklch(64% 0.01 40)";
  const p = project as PublicProject;

  return (
    <div style={{ minHeight: "100vh", background: bg, color: "oklch(90% 0.008 40)", fontFamily: "var(--ff-body)", overflowY: "auto" }}>
      <div style={{ borderBottom: `1px solid ${border}`, padding: "1.25rem 2.5rem", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: bg, zIndex: 10 }}>
        <BackButton onClose={onClose} color={accent} />
        <span style={{ fontFamily: "var(--ff-mono)", fontSize: "0.65rem", color: accent, letterSpacing: "0.1em" }}>RECIPE RELAY · 2024</span>
        <GitHubLink url={p.sourceLink} accent={accent} />
      </div>
...
function ConfyDetail({ project, onClose }: { project: AnyProject; onClose: () => void }) {
  const accent = "oklch(65% 0.2 145)";
  const bg     = "oklch(11% 0.008 145)";
  const surface= "oklch(15% 0.01 145)";
  const border = "oklch(22% 0.01 145)";
  const muted  = "oklch(60% 0.01 145)";
  const p = project as PublicProject;

  return (
    <div style={{ minHeight: "100vh", background: bg, color: "oklch(90% 0.008 145)", fontFamily: "var(--ff-body)", overflowY: "auto" }}>
      <div style={{ borderBottom: `1px solid ${border}`, padding: "1.25rem 2.5rem", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: bg, zIndex: 10 }}>
        <BackButton onClose={onClose} color={accent} />
        <span style={{ fontFamily: "var(--ff-mono)", fontSize: "0.65rem", color: accent, letterSpacing: "0.1em" }}>CONFYSPRING · 2024</span>
        <GitHubLink url={p.sourceLink} accent={accent} />
      </div>
...
function DelawareDetail({ project, onClose }: { project: AnyProject; onClose: () => void }) {
  const accent = "oklch(58% 0.2 25)";
  const bg     = "oklch(10% 0.008 20)";
  const surface= "oklch(14% 0.01 20)";
  const border = "oklch(22% 0.01 20)";
  const muted  = "oklch(60% 0.01 20)";
  const p = project as PublicProject;

  return (
    <div style={{ minHeight: "100vh", background: bg, color: "oklch(90% 0.006 20)", fontFamily: "var(--ff-body)", overflowY: "auto" }}>
      <div style={{ borderBottom: `1px solid ${border}`, padding: "1.25rem 2.5rem", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: bg, zIndex: 10 }}>
        <BackButton onClose={onClose} color={accent} />
        <span style={{ fontFamily: "var(--ff-mono)", fontSize: "0.65rem", color: accent, letterSpacing: "0.1em" }}>DELAWARE · 2025</span>
        <GitHubLink url={p.sourceLink} accent={accent} />
      </div>
...
function WebscraperDetail({ project, onClose }: { project: AnyProject; onClose: () => void }) {
  ...
  const bg     = "oklch(10% 0.01 290)";
  const surface= "oklch(14% 0.012 290)";
  const border = "oklch(22% 0.012 290)";
  const muted  = "oklch(60% 0.01 290)";
  const p = project as PrivateProject;

  return (
    <div style={{ minHeight: "100vh", background: bg, color: "oklch(90% 0.006 290)", fontFamily: "var(--ff-body)", overflowY: "auto" }}>
      <div style={{ borderBottom: `1px solid ${border}`, padding: "1.25rem 2.5rem", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: bg, zIndex: 10 }}>
        <BackButton onClose={onClose} color={accent} />
        <span style={{ fontFamily: "var(--ff-mono)", fontSize: "0.65rem", color: accent, letterSpacing: "0.1em" }}>WEBSCRAPER · 2026</span>
        <PrivateBadge client={p.client} />
      </div>
...
function WebsearcherDetail({ project, onClose }: { project: AnyProject; onClose: () => void }) {
  const accent = "oklch(60% 0.17 265)";
  const bg     = "oklch(10% 0.01 265)";
  const surface= "oklch(14% 0.012 265)";
  const border = "oklch(22% 0.012 265)";
  const muted  = "oklch(60% 0.01 265)";
  const p = project as PrivateProject;

  return (
    <div style={{ minHeight: "100vh", background: bg, color: "oklch(90% 0.006 265)", fontFamily: "var(--ff-body)", overflowY: "auto" }}>
      <div style={{ borderBottom: `1px solid ${border}`, padding: "1.25rem 2.5rem", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: bg, zIndex: 10 }}>
        <BackButton onClose={onClose} color={accent} />
        <span style={{ fontFamily: "var(--ff-mono)", fontSize: "0.65rem", color: accent, letterSpacing: "0.1em" }}>WEBSEARCHER · 2026</span>
        <PrivateBadge client={p.client} />
      </div>
...
function EchoDetail({ project, onClose }: { project: AnyProject; onClose: () => void }) {
  const accent = "oklch(64% 0.16 185)";
  const bg     = "oklch(10% 0.01 185)";
  const surface= "oklch(14% 0.012 185)";
  const border = "oklch(22% 0.012 185)";
  const muted  = "oklch(60% 0.01 185)";
  const p = project as PrivateProject;

  return (
    <div style={{ minHeight: "100vh", background: bg, color: "oklch(90% 0.006 185)", fontFamily: "var(--ff-body)", overflowY: "auto" }}>
      <div style={{ borderBottom: `1px solid ${border}`, padding: "1.25rem 2.5rem", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, background: bg, zIndex: 10 }}>
        <BackButton onClose={onClose} color={accent} />
        <span style={{ fontFamily: "var(--ff-mono)", fontSize: "0.65rem", color: accent, letterSpacing: "0.1em" }}>ECHO · 2026</span>
        <PrivateBadge client={p.client} />
      </div>
...
export function DetailOverlay({ project, onClose }: { project: AnyProject | null; onClose: () => void }) {
  ...
  return (
    <>
      ...
      >
        {displayed && <ProjectDetailPage project={displayed} onClose={onClose} />}
      </div>
    </>
  );
}
